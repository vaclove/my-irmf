...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (photo && photo.trim()) {
      try {
        // Upload photo to S3
        const uploadResult = await guestImageStorage.migrateBase64Image(photo, id);
        imagePath = uploadResult.basePath;
        imageMigrated = true;
        
        // Update guest with S3 path and photo data
        const result = await pool.query(
          `UPDATE guests SET 
           first_name = $1, 
           last_name = $2, 
           ...
           WHERE id = $13 
           RETURNING *`,
          [first_name, last_name, email, phone, guestLanguage, company, notes, finalGreeting, greetingAutoGenerated, photo, imagePath, imageMigrated, id]
        );
        ...
        res.json(responseGuest);
        
      } catch (uploadError) {
        console.error('Failed to upload guest photo to S3:', uploadError);
        // If S3 upload fails, still update the guest with database photo as fallback
        const result = await pool.query(
          `UPDATE guests SET 
           first_name = $1, 
           last_name = $2, 
      ...
      // No photo provided - update without photo changes
      const result = await pool.query(
        `UPDATE guests SET 
         first_name = $1, 
         last_name = $2, 
         ...
         WHERE id = $10 
         RETURNING *`,
        [first_name, last_name, email, phone, guestLanguage, company, notes, finalGreeting, greetingAutoGenerated, id]
      );
      ...
      // Transform response to include S3 URLs if guest has image_path
      const responseGuest = { ...result.rows[0] };
      delete responseGuest.photo; // Remove base64 data
      
      if (result.rows[0].image_path) {
        responseGuest.image_urls = {
          thumbnail: guestImageStorage.getImageUrl(result.rows[0].image_path, 'thumbnail'),
          medium: guestImageStorage.getImageUrl(result.rows[0].image_path, 'medium'),
          original: guestImageStorage.getImageUrl(result.rows[0].image_path, 'original')
        };
      }
